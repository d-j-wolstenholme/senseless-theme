...
1. **`./scripts/deploy.sh $FILES` under zsh does NOT word-split.** deploy.sh received 25 paths as ONE `--only` argument, printed **"deploy: success"**, and pushed **nothing**. Run it under `bash`, and always verify per file via the Asset API. This is the project's known deployed≠committed failure mode in a new disguise.
2. **`assets/images/**` DOES deploy.** 18 files (~7.5 MB of unreferenced source PNGs) are live theme assets. Any verifier built on "they never deploy" mis-counts.
3. **Verify parity with the Admin Asset API, not just `theme pull`.** `themes.json` → `updated_at` is a one-call check: if it hasn't moved since the last verified parity, no Theme-Editor write happened.
4. **The version invariant has no repo-side enforcement.** `.claude/hooks/guard-write.py:47` only checks `canon_version` is truthy, never that it equals `canon/state.json`. That is why `schema-contract.json` sat at v2.19 through a version sweep.
5. **Liquid raises on a nil comparison** — `image.aspect_ratio` can be nil for SVGs. Guard with `| default: 0`.
6. **The PDP 45–60 minute line is NOT a breach** — operator-accepted under Decision `39158bc3-75ea-81f7`, recorded in `docs/COMPLIANCE.md:40` and the compliance-check skill as explicitly "not a regression". An earlier note cited `…-8181` and mis-stated its scope. Rewriting it would reverse a live operator decision.
7. **`/pages/choosing-your-format` is retired** (301, not in the sitemap). Do not harvest copy from it — its "gel spreads further" contradicts every live gel surface.
8. **Clinical gel SKUs are the anomaly, not the docs.** Live `S15CL`/`S35CL` vs `SG15AD`/`SG15PR` on the other gels. `docs/ARCHITECTURE.md` is internally consistent; Daniel decides whether live gets renamed.
9. **Two repo scripts are destructive if re-run:** `policy-menus-redirects.py` does a full `menuUpdate` replace against a stale record and would **wipe Articles + Rewards from the primary nav**; `policy-metafields.py` would publish shipping prices **£1 under** what checkout charges.
10. **The State Surface log is past its rollover point** — 66 entries against Blueprint 01 §4.7's ~20, ~94% of the page. The operator deferred the backlog at v2.19; worth raising again.
