...
- **`03-hub.png` must NOT go on the website without a decision** — it contains a "Numbing timing — When to apply and how long it lasts" row. Duration-as-claim breaches the Hard Rules on a brand surface (it's acceptable inside the app's own store listing, which is why it ships there).
