...
- **Per-machine `.env`:** the client credentials live in the git-ignored `.env`, which does not travel with the repo. **On the Mac mini they were EMPTY (checked 21 Sep 2026)**, so `refresh-token.sh` and `deploy.sh` (exits at `deploy.sh:118-120`) fail there until the MacBook Pro `.env` is copied across. The Shopify CLI's own login (`shopify theme list/pull --store senseless-numbing.myshopify.com`) still works for the store gate and reading live. A missing `.env` is a machine-setup step, not a stakeholder blocker.
