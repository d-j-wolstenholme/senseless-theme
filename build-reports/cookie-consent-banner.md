...
> **CORRECTION (21 Sep 2026):** item 1 below was misdiagnosed. The cause is the banner's own code, not a store setting. `snippets/senseless-cookie-consent.liquid:127` passes `sale_of_data` as the string `'enabled'`/`'disabled'`. The Storefront API expects a Boolean and rejects the whole consent write ("Argument 'saleOfData' on InputObject 'VisitorConsent' has an invalid value … Expected type 'Boolean'"). That leaves no `data`, which surfaces as "reading 'consentManagement'". It was reproduced twice in headless Chrome on 21 Sep, for both Accept and Reject. Consent has never been recorded since launch. Fix: `sale_of_data: !!c.marketing`. Timing is the founder's call (it changes what Google Ads/GA4 record). See `docs/GOOGLE-ADS-MC-AUDIT-2026-09-20.md` N1/V53 and §5A item 2.
