...
| `senseless-structured-data.liquid` | JSON-LD dispatcher rendered in `theme.liquid` head. Emits Product + one selected-variant Offer on PDPs (and on every `?variant=` URL); ProductGroup + nested hasVariant (one Offer per size) on the BASE URL of two-size PDPs (since `d04c60d`, 24 Sep 2026) and CollectionPage+ItemList (collections) via the `json` filter (live price/availability/currency; seller legalName "Matrix Health Group Ltd"; claim-free). Dispatches on `request.page_type`; does **not** duplicate Organization/FAQPage/Article schema emitted elsewhere. |
...
| `senseless-app-banner.liquid` | Cart app banner, rendered from `cart-summary.liquid` so it appears in drawer + cart page, directly above the checkout CTAs (TN-cart placement). **Mobile-layout only** — strictly ≤749px (Daniel's call 28 Jul: it tracks "the mobile version of the site", so tablets and landscape phones on the desktop layout are knowingly excluded; don't re-raise). 10% price maths (was/now, integer pence) — suppressed when the cart already carries any discount (misleading-price guard); double-points + welcome-bonus-on-app-sign-in line; OS-picked store badge. All claims live-verified (discount ACTIVE via Admin API; loyalty facts = live rewards page). |
